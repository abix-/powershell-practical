...
foreach($_d in $dvs) {
    ...
    try { WriteHost "Getting port group data for $($_d.name)"; $dvpgs = Get-VDPortgroup -VDSwitch $_d -ErrorAction Stop | ?{$_.Name -like "*vmMGMT*" -or $_.Name -like "*vMotion*"} | sort Name }
    ...
    foreach($_v in $vmk) {
        try {
            ...
            $pg_array += New-VirtualPortGroup -VirtualSwitch $vss -Name $_v.PortGroupName -VLanId $vlan -ErrorAction Stop
        }
