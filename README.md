...
* **Test-PortGroups**: validates the trunking configuration of VMHosts by moving a VM between every PortGroup on a VMHost. The VM will Set-VMGuestNetwork with an IP valid for each PortGroup then pinged with Test-Connection and then moved to the next PortGroup with Set-NetworkAdapter. A ping failure likely indicates a trunking/networking misconfiguration
