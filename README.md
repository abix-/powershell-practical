...
* **Start-ApplicanceHealthReport**: creates HTML report on the disk usage, service status, ssh status, and vSphere web client response time for vSphere VMs with Invoke-VMScript and Regex. Uses Get-Service and Get-WMIObject on Windows. Uses df, chkconfig, and systemctl on Linux<br>
